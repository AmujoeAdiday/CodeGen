...
# Load the Excel file from local drive
excel_path = r"C:\Data\Model_Suggestion.xlsx"

# Debug section - remove after fixing
st.sidebar.subheader("Debug Info")
st.sidebar.write(f"Looking for file at: {excel_path}")
st.sidebar.write(f"File exists: {os.path.exists(excel_path)}")
if not os.path.exists(excel_path):
    st.sidebar.write(f"Current directory: {os.getcwd()}")

try:
    xl_data = pd.read_excel(excel_path, sheet_name="Overall Suggestion", engine='openpyxl')
    st.success("✓ Excel file loaded successfully!")
except FileNotFoundError:
    st.error(f"❌ File not found: {excel_path}")
    st.info("Make sure the file exists at C:\\Data\\Model_Suggestion.xlsx")
    xl_data = None
...
if st.button("Generate Formula"):
    if xl_data is not None:
        ...
        st.warning("⚠️ Cannot generate formula - Excel file not loaded. Check file path above.")
